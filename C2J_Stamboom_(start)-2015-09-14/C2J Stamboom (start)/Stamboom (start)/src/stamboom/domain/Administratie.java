...
public class Administratie implements Serializable{
    ...
    public Gezin addHuwelijk(Persoon ouder1, Persoon ouder2, GregorianCalendar huwdatum) {
        ...
        {
            if((family.getOuder1() == ouder1 && family.getOuder2() == ouder2) || family.getOuder1() == ouder2 && family.getOuder2() == ouder1)
            {
                if(family.isOngehuwd())
                {
                    if(ouder1.kanTrouwenOp(huwdatum) && ouder2.kanTrouwenOp(huwdatum))
                    {
                        family.setHuwelijk(huwdatum);
                        result = family;
                    }
                }
            }
        ...
        if(result == null)
        {
            if(ouder1.kanTrouwenOp(huwdatum) && ouder2.kanTrouwenOp(huwdatum))
            {
                Gezin family = new Gezin(nextGezinsNr, ouder1, ouder2);
                family.setHuwelijk(huwdatum);   
                result = family;
            }
        }
    ...
    public ArrayList<Persoon> getPersonenMetAchternaam(String achternaam) {
        ...
        for(Persoon person : personen)
        {
            if(person.getAchternaam() == achternaam)
            {
                result.add(person);
            }
        }
    ...
    public Persoon getPersoon(String[] vnamen, String anaam, String tvoegsel,
        ...
        for(Persoon person : personen)
        {
            if(vnamen.toString().contains(person.getVoornamen()) && person.getAchternaam() == anaam && person.getTussenvoegsel() == tvoegsel && person.getGebDat() == gebdat && person.getGebPlaats() == gebplaats)
            {
                result = person;
            }
        }
