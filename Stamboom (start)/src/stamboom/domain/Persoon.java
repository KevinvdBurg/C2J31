...
public class Persoon implements Serializable {
    ...
    Persoon(int persNr, String[] vnamen, String anaam, String tvoegsel,
        ...
        for (int i = 0; i < vnamen.length; i++) {
            vnamen[i] = vnamen[i].toLowerCase().substring(0, 1).toUpperCase() + vnamen[i].toLowerCase().substring(1);
        }
        voornamen = vnamen;
        achternaam = anaam.toLowerCase().substring(0, 1).toUpperCase() + anaam.toLowerCase().substring(1);
        tussenvoegsel = tvoegsel.toLowerCase();
        gebDat = gebdat;
        gebPlaats = gebplaats.toLowerCase().substring(0, 1).toUpperCase() + gebplaats.toLowerCase().substring(1);
    ...
    boolean setOuders(Gezin ouderlijkGezin) {
        ...
        if (!ouderlijkGezin.getKinderen().contains(this)) {
    ...
    public boolean kanTrouwenOp(Calendar datum) {
        ...
        for (Gezin gezin : alsOuderBetrokkenIn) {
            if (gezin.heeftGetrouwdeOudersOp(datum)) {
                ...
                if (huwdatum != null && huwdatum.after(datum) && !gezin.heeftGescheidenOudersOp(datum)) {
                    return false;
                }
            }
        }
    ...
    public boolean isGescheidenOp(Calendar datum) {
        //todo opgave 1
        return this.ouderlijkGezin.getScheidingsdatum() == datum;
    }
