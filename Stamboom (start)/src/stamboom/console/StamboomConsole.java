...
public class StamboomConsole {
    ...
    void saveAdministratie()
    {
        ...
        path = path.substring(0, path.length() - 24) + "\\administratieFiles";
        File newFile = new File(path, readString("Bestandslocatie") + ".administratie");
        try {
            controller.serialize(newFile);
    ...
    void loadAdministratie()
    {
        String path = getClass().getProtectionDomain().getCodeSource().getLocation().getPath();
        path = path.substring(0, path.length() - 24) + "\\administratieFiles";
        File loadedFile = new File(path, readString("Bestandslocatie") + ".administratie");
        try {
            controller.deserialize(loadedFile);
        } catch (IOException ex) {
            System.out.println("Loading failed: " + ex.getMessage());
        }
    }
    
    void genereerStamboom()
    {
        int Persoonsnummer = readInt("Persoonsnummer");
        String stamboom = controller.getAdministratie().getPersoon(Persoonsnummer).stamboomAlsString();
        System.out.print(stamboom);
    }
}
